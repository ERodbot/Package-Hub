CREATE OR ALTER PROCEDURE usp_ReportingByPerformance
	...
	@country_filter VARCHAR(50) = NULL
AS 
BEGIN
...
-- Validacion de rol_filter si se proporciona
IF @role_filter IS NOT NULL
BEGIN
    IF NOT EXISTS (SELECT 1 FROM hr.[human-resources]..Role WHERE Role.name = @role_filter)
    BEGIN 
        ...
        SET @CustomError = 50001 -- Puedes definir un n�mero de error personalizado seg�n tus necesidades
        RAISERROR('%s - Error Number: %i', @ErrorSeverity, @ErrorState, @Message, @CustomError)
        RETURN
    END
END

-- Validacion de country_filter si se proporciona
IF @country_filter IS NOT NULL
BEGIN
    IF NOT EXISTS (SELECT 1 FROM hr.[human-resources]..Country WHERE Country.name = @country_filter)
    BEGIN 
        SET @Message = 'Error - El pa�s especificado no existe en la base de datos.'
        SET @ErrorSeverity = 16 -- Puedes ajustar el nivel de severidad seg�n tus necesidades
...
END
    ...
    -- Verificar si no hay una transaccion en curso
    IF @@TRANCOUNT = 0
    BEGIN
    ...
    emp.lastName,
    emmper.rating,
    pm.name AS performanceMetric,
    ci.phone,
    ci.email,
    dep.name AS department,  -- This line seems to be causing the issue
...
INNER JOIN  
    hr.[human-resources]..ContactInfo ci ON ci.idContactInfo = emp.idContactInfo
INNER JOIN  
...
INNER JOIN  
    hr.[human-resources]..PerformanceMetrics pm ON pm.idPerformanceMetrics = emmper.idPerformanceMetrics
WHERE 
        (@initial_date IS NULL OR emmper.startDate BETWEEN @initial_date AND @final_date)
        AND (@final_date IS NULL OR emmper.endDate BETWEEN emmper.startDate AND @final_date)
        AND (@role_filter IS NULL OR emp.idRole IN (SELECT idRole FROM hr.[human-resources]..Role WHERE Role.name = @role_filter))
        AND (@country_filter IS NULL OR st.idCountry IN (SELECT idCountry FROM hr.[human-resources]..Country WHERE Country.name = @country_filter))
